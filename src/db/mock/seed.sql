...
-- spin_segments duplikatlarini tozalash (migration bir necha marta ishlagan bo'lsa)
DELETE FROM spin_results WHERE segment_id NOT IN (
  SELECT MIN(id) FROM spin_segments GROUP BY sort_order
);
DELETE FROM spin_segments WHERE id NOT IN (
  SELECT MIN(id) FROM spin_segments GROUP BY sort_order
);
